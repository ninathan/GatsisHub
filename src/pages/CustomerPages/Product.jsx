            ...
            <div className="max-w-6xl mx-auto px-6 py-16">
                ...
                <div className="space-y-24">
                    {products.map((product) => (
                        <div
                            key={product.id}
                            className={`flex items-center gap-12 ${product.alignment === 'right' ? 'flex-row-reverse' : ''
                                }`}
                        >
                            {/* Product Image */}
                            <div className="flex-1 flex justify-center">
                                <div className="text-9xl">
                                    {product.id === 'MB7' ? <img src={product.image} alt={product.name} /> : <img src={product.image} alt={product.name} />}
                                </div>
                            </div>
                            ...
                            <div className={`flex-1 ${product.alignment === 'right' ? 'text-right' : ''}`}>
                                ...
                                {product.hasOrderButton && (
                                    <Link to="/checkout">
                                        <button className="mt-6 bg-yellow-400 hover:bg-yellow-500 text-black font-semibold px-8 py-3 rounded-full transition-colors">
                                            Order Now
                                        </button>
                                    </Link>
                                )}
